...
*              functions which are part of the PMG1 MCU USB-PD Sink PPS demo
...
static uint32_t calc_power(uint32_t voltage, uint32_t current)
{
    /*
       ...
       eurrent is expressed in 10 mA units.
...
#if (CY_PD_PPS_SNK_ENABLE)
        /* If source PDO is Augumented type */
        case CY_PDSTACK_PDO_AUGMENTED:
            /* If source augumented PDO type is PPS */
            if (pdo_src->pps_src.apdoType == CY_PDSTACK_APDO_PPS)
            {
                /* Assigning the maximum current from the PPS source PDOs */
                gl_pps_src_pdo.min_curr = pdo_src->pps_src.maxCur;

                /* Assigning the minimum voltage from the PPS source PDOs */
                gl_pps_src_pdo.min_volt = pdo_src->pps_src.minVolt;

                /* Reading the maximum PPS voltage and object position
                 * among the PPS source PDOs */
                if (pdo_src->pps_src.maxVolt > gl_pps_src_pdo.max_volt)
                {
                    gl_pps_src_pdo.max_volt = pdo_src->pps_src.maxVolt;
                }

                /* Reading the minimum PPS current among the PPS source PDOS */
                if (gl_pps_src_pdo.min_curr < pdo_src->pps_src.maxCur)
                {
                    gl_pps_src_pdo.min_curr = pdo_src->pps_src.maxCur;
                }
        ...
        cy_pdstack_app_resp_cbk_t app_resp_handler)
{
    ...
    for(snk_pdo_index = 0u; snk_pdo_index < context->dpmStat.curSnkPdocount; snk_pdo_index++)
    {
        for(src_pdo_index = 0u; src_pdo_index < srcCap->len; src_pdo_index++)
        {
            ...
            if (ppsSetFlag == true)
            {
                gl_pps_src_pdo.obj_position = src_pdo_index + 1;
            }
#endif
        }
    }
